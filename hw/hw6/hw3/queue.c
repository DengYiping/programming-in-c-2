...
static void printq_helper(Node* head){
    //print in a recursive backward fasion
    if(head == NULL)
        return;
    printq_helper(head->next); //depth-first print
    printf("%d ", head->item);
    return;
}
...
void printq(Queue* pq){
    //just a wrapper function. Actual job is done in printq_helper
    if(pq == NULL)
        return;
    printf("content of the queue: ");
    printq_helper(pq->rear); //just call it
    printf("\n");
}
